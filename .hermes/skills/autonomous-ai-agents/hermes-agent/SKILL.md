...
### ⚠️ Pitfall: delegation.api_key vs .env DEEPSEEK_API_KEY

The delegation system reads the API key from **`~/.hermes/.env`** (`DEEPSEEK_API_KEY` env var), NOT from `config.yaml`'s `delegation.api_key` field. Setting `delegation.api_key` in config.yaml does nothing for subagents.

**To update the delegation API key:**
```bash
...
The `delegation.api_key` in config.yaml is a separate field that may be used by some code paths, but the primary source is the .env file. Always update BOTH to be safe.
...
### Handling User Corrections to Plugins

When a user corrects a plugin's behavior (e.g., "关掉思考太降智了"):

**Step 1: Locate the plugin file**
```bash
# Find plugin by name
search_files(pattern="xiaomi", target="files", path="/root/.hermes/hermes-agent/plugins")
```

**Step 2: Read and understand current implementation**
```python
read_file(path="/root/.hermes/hermes-agent/plugins/model-providers/xiaomi/__init__.py")
```

**Step 3: Make targeted changes**
- Use `patch()` tool for precise edits
- Remove or modify the problematic functionality
- Keep the plugin structure intact

**Step 4: Commit the change**
```bash
cd /root/.hermes/hermes-agent
git add plugins/model-providers/xiaomi/__init__.py
git commit -m "remove thinking control from xiaomi plugin - avoid降智"
```

**Example: Removing thinking control from xiaomi plugin**
- User correction: "mimo的思考插件去掉吧，关掉思考太降智了"
- Issue: Plugin was forcing `thinking: {"type": "enabled/disabled"}` which降智
- Fix: Remove `build_api_kwargs_extras()` method entirely
- Result: MiMo uses default behavior, no forced thinking control

**Pitfall:** Don't just comment out code - remove it completely to avoid confusion.
